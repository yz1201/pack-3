...
public class VerifyCodeServiceImpl implements IVerifyCodeService {
    ...
    public ResponseResult generateVerificationCode(Integer identity, String phoneNumber) {
        ...
        if (shouldLimitAtLastMin(minKey)) {
            return new ResponseResult<VerifyCode>().setCode(CommonStatusEnum.VERIFICATION_ONE_MIN_SEND_ERROR.getCode())
                    .setMessage(CommonStatusEnum.VERIFICATION_ONE_MIN_SEND_ERROR.getValue());
        }

        if (shouldLimitAtLastHour(hourKey)) {
            return new ResponseResult<VerifyCode>().setCode(CommonStatusEnum.VERIFICATION_ONE_HOUR_SEND_ERROR.getCode())
                    .setMessage(CommonStatusEnum.VERIFICATION_ONE_HOUR_SEND_ERROR.getValue());
        }
